...
def render(data: dict, title: str, generated_at: str) -> str:
    ...
    ps = data.get("pricing_suggestion", {})
    mc = data.get("market_comparison", {})
    risks = data.get("risks", [])
    summary = data.get("summary", "")
    vram = data.get("vram_check", {})

    # ---- KPI ----
    margin = ps.get("gross_margin", 0)
    kpi_cards = (
        ...
        f'<div class="kpi"><div class="v">{fmt_num(ps.get("input_price_per_million", 0))} / {fmt_num(ps.get("output_price_per_million", 0))}</div><div class="l">建议售价（输入 / 输出）</div></div>'
        f'<div class="kpi"><div class="v">{fmt_pct(margin)}</div><div class="l">目标毛利率</div></div>'
    )

    # ---- 成本结构 ----
    rows_cost = (
        f'<tr><td>单卡月成本（输入）</td><td class="num">{fmt_money(cm.get("single_card_monthly_cost", 0))}</td><td>含硬件摊销、电费、机房、运维；折旧期 {cm.get("depreciation_years", "-")} 年</td></tr>'
        f'<tr><td>÷ 利用率 {fmt_pct(cm.get("utilization", 0))} → 有效成本</td><td class="num">{fmt_money(cm.get("effective_card_cost_per_month", 0))}</td><td>空转部分为沉没成本</td></tr>'
        f'<tr><td>×（1 + {fmt_pct(cm.get("overhead_ratio", 0.2))}）→ 总月成本</td><td class="num">{fmt_money(cm.get("total_monthly_cost", 0))}</td><td>电费/带宽/存储/集群/运维</td></tr>'
    ...
    # ---- 定价体系 ----
    tiers = ps.get("price_tiers_impact", {})
    cache = tiers.get("cache_discount", {})
    batch = tiers.get("batch_discount", {})
    pkg = tiers.get("package_check", {})
    ...
    max_price = 100.0
    competitors = []
    dsp = mc.get("deepseek_v4_pro_official", {})
    if dsp:
    ...
    if not rows_risk:
        rows_risk = '<tr><td colspan="2" class="ok-cell">无风险</td></tr>'
    ...
    if not action_items:
        action_items.append(("P1", "按 summary 结论细化行动项"))
    rows_action = ""
...
<div class="wrap">
  ...
  <h1>{esc(title)}</h1>
  <div class="sub">商业简报 ｜ 测算日期 {generated_at} ｜ 部署：{esc(cm.get("gpu_type","—"))} ｜ 模型：{esc(cm.get("model","—"))} ｜ 目标毛利 {fmt_pct(margin)}</div>
  ...
  <h2>② 建议定价体系（{fmt_pct(margin)} 毛利）</h2>
  ...
  <table><tr><th>利用率</th><th class="num">输出成本（¥/百万）</th><th class="num">对应售价（{fmt_pct(margin)} 毛利）</th></tr>{rows_sens}</table>
